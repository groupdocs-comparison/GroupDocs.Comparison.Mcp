# Release Process — GroupDocs.Comparison.Mcp
...
2. **Bump both `.mcp/server.json` versions** — [src/GroupDocs.Comparison.Mcp/.mcp/server.json](src/GroupDocs.Comparison.Mcp/.mcp/server.json) has **two** `version` fields that must both match:
...
3. **Update pinned versions in `README.md`** — search for the old version and replace every occurrence (`dnx GroupDocs.Comparison.Mcp@…`, Claude Desktop config, VS Code `mcp.json`). Typically 4 places.
...
5. *(Optional, rare)* Bump external dependency versions in the same props file — `GroupDocsMcpCore`, `GroupDocsComparison`, `MicrosoftExtensionsHosting`, `ModelContextProtocol`, `MicrosoftSourceLinkGithub`.
...
2. Uses GitHub OIDC to authenticate with the MCP Registry — namespace `io.github.groupdocs-comparison/*` is auto-verified because the repo lives under `github.com/groupdocs-comparison/*`.
...
The `publish_mcp_registry` job authenticates via GitHub OIDC. The Registry auto-verifies the `io.github.groupdocs-comparison/*` namespace because the workflow runs inside `github.com/groupdocs-comparison/*` — no DNS records, no manual tokens, no opt-in variable required. First-time publish claims the namespace automatically; subsequent releases update it.

If you ever fork the repo to a different org (e.g. `github.com/someone-else/*`), the Registry publish will fail until either the `server.json` `name` field is updated to match the new org's namespace or DNS verification is added — but for the canonical `groupdocs-comparison` org, it just works.
...
4. Bump `N` in `dependencies.props` + `.mcp/server.json` (e.g. `26.5.0` → `26.5.1`).
